...
TextAttack can attacks any model that takes a list of strings as input and outputs a list of predictions. This is the idea behind *model wrappers*: to help your model conform to this API, we've provided the ``textattack.models.wrappers.ModelWrapper`` abstract class.
...
TextAttack also provides lots of pre-trained models for common tasks. Testing different attacks on the same model ensures attacks comparisons are fair.

Any of these models can be provided to ``textattack attacks`` via ``--model``, for example, ``--model bert-base-uncased-mr``. For a full list of pre-trained models, see the `pre-trained models README <https://github.com/QData/TextAttack/tree/master/textattack/models>`_.
