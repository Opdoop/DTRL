...
We provide a number of pre-built attacks recipes, which correspond to attacks from the literature. To run an attacks recipe from the command line, run::

    textattack attacks --recipe [recipe_name]

To initialize an attacks in Python script, use::

    <recipe name>.build(model_wrapper)

For example, ``attacks = InputReductionFeng2018.build(model)`` creates `attacks`, an object of type ``Attack`` with the goal function, transformation, constraints, and search method specified in that paper. This object can then be used just like any other attacks; for example, by calling ``attacks.attack_dataset``.

TextAttack supports the following attacks recipes (each recipe's documentation contains a link to the corresponding paper):
