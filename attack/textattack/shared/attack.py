...
- `Goal Functions <../attacks/goal_function.html>`__ stipulate the goal of the attacks, like to change the prediction score of a classification model, or to change all of the words in a translation output.
...
The ``Attack`` class represents an adversarial attacks composed of a goal function, search method, transformation, and constraints.
"""
...
class Attack:
    """An attacks generates adversarial examples on text.

    This is an abstract class that contains main helper functionality for
    attacks. An attacks is comprised of a search method, goal function,
    a transformation, and a set of one or more linguistic constraints that
    successful examples must meet.

    Args:
        goal_function: A function for determining how well a perturbation is doing at achieving the attacks's goal.
        constraints: A list of constraints to add to the attacks, defining which perturbations are valid.
        transformation: The transformation applied at each step of the attacks.
    ...
    def __init__(
        ...
        if not self.goal_function:
            raise NameError(
                "Cannot instantiate attacks without self.goal_function for predictions"
            )
        self.search_method = search_method
        if not self.search_method:
            raise NameError("Cannot instantiate attacks without search method")
        self.transformation = transformation
        if not self.transformation:
            raise NameError("Cannot instantiate attacks without transformation")
        self.is_black_box = (
        ...
        # by the attacks class when checking whether to skip the sample
    ...
    def _get_transformations_uncached(self, current_text, original_text=None, **kwargs):
        ...
        Args:
            ...
            original_text: The original ``AttackedText`` from which the attacks started.
        Returns:
    ...
    def _filter_transformations_uncached(
        ...
        for C in self.constraints:
            try:
                ...
                # Some weird symbol as '>>>' cause IndexError Tyr to pass this example
                pass
    ...
    def _get_examples_from_dataset(self, dataset, indices=None):
        ...
        Args:
            ...
            indices: An iterable of indices of the dataset that we want to attacks. If None, attacks all samples in dataset.
        ...
        while indices:
            ...
            try:
                ...
                utils.logger.warn(
                    f"Dataset has {len(dataset)} samples but tried to access index {i}. Ending attacks early."
                )
                break
    ...
    def attack_dataset(self, dataset, indices=None):
        """Runs an attacks on the given dataset and outputs the results to the
    ...
    def __repr__(self):
        """Prints attacks parameters in a human-readable string.
