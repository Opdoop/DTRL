...
class Checkpoint:
    ...
    Args:
        args: Command line arguments of the original attacks
        log_manager (AttackLogManager): Object for storing attacks results
    ...
    def __repr__(self):
        ...
        attack_logger_lines.append(
            utils.add_indent(
                f"(Total number of examples to attacks): {self.args.num_examples}", 2
            )
        )
        attack_logger_lines.append(
        ...
        lines.append(
            utils.add_indent(f"(Previous attacks summary):  {attack_logger_str}", 2)
        )
