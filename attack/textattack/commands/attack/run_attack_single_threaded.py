...
A command line parser to run an attacks in single thread from user specifications.
...
from .attack_args_helpers import (
    parse_attack_from_args,  # 传入哪些参数来获取 attacks 模型
    parse_dataset_from_args,  # 传入哪些参数来获取 dataset
    parse_logger_from_args,  # logger 具体负责哪些事情
)
...
def run(args, checkpoint=None):
    ...
    if args.interactive:
        ...
        while True:
            print('Enter a sentence to attacks or "q" to quit:')
            text = input()
        ...
        for result in attack.attack_dataset(dataset, indices=worklist):  # 上面创建的 deque: worklist 作为下标
            ...
            if (not args.attack_n) or ( # attack_n 为 false
                    not isinstance(result, textattack.attack_results.SkippedAttackResult)  # 或得到的 attacks 样本的结果是 skip
            ):
