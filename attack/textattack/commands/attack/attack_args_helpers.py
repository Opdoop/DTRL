...
def add_model_args(parser):
    ...
    model_group.add_argument(
        ...
        required=False,
        default=None,
        help='Name of or path to a pre-trained model to attacks. Usage: "--model {model}:{arg_1}={value_1},{arg_3}={value_3},...". Choices: '
        + str(model_names),
    )
    model_group.add_argument(
        "--model-from-file",
...
def parse_attack_from_args(args):
    ...
    if args.recipe:  # 如果指定了 Attack recipe
        if ARGS_SPLIT_TOKEN in args.recipe:  # 如果 recipe 有 '^' 切断标记
            ...
            recipe = eval(f"{ATTACK_RECIPE_NAMES[recipe_name]}.build(model, {params})")  # 使用 eval 创建 attacks 模型，为何使用 eval 不清楚。
        elif args.recipe in ATTACK_RECIPE_NAMES:  # 单纯的传入了 recipe
            recipe = eval(f"{ATTACK_RECIPE_NAMES[args.recipe]}.build(model)")  # 使用 eval 创建 attacks 模型
        else:  # 没用传入 recipe 参数
        ...
        if ARGS_SPLIT_TOKEN in args.attack_from_file:
            ...
            attack_file, attack_name = args.attack_from_file, "attacks"
        ...
        if ARGS_SPLIT_TOKEN in args.search:  # 如果有 '^' 切割符
            ...
            raise ValueError(f"Error: unsupported attacks {args.search}")  # 报错
...
def parse_model_from_args(args):
    ...
    if args.model_from_file:  # 从指定的 .py 文件读取模型
        ...
        try:
            model = getattr(model_module, model_name)  # 尝试从创建的 attacks 模型对象中获取其 model_name 参数
