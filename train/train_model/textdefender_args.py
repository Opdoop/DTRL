...
class ProgramArgs(argparse.Namespace):
    def __init__(self):
        ...
        self.mode = 'train'  # in ['train', 'attacks', 'evaluate', 'augment', 'textattack_augment', 'dev_augment']
        self.dataset_name = 'mr'
        self.keep_sentiment_word = 'False'
        self.model_name_or_path = '../../../bert-base-uncased'
        self.model_type = 'bert' # base encoder
        self.evaluation_data_type = 'test'
        self.training_type = 'infobert'

        # attacks parameters
        # self.attack_method = 'bae'
        # self.attack_times = 1
        # self.attack_numbers = 1000
        # # attacks constraint args defined by us
        ...
        # path parameters
        self.workspace = '/root/zengdajun_2_1/data/zjh/DisenADA'
    ...
    def build_saving_dir(self):
        self.saving_dir = os.path.join(self.saving_dir, "{}_{}".format(self.dataset_name, self.training_type))
        check_and_create_path(self.saving_dir)

    def build_logging_dir(self):
        self.log_path = f"{self.workspace}/outputs/training/{self.dataset_name}-{self.training_type}-{self.model_type}"
    ...
    def build_caching_dir(self):
        ...
        self.caching_dir = os.path.join(self.caching_dir, "{}_{}".format(self.dataset_name, self.training_type))
        check_and_create_path(self.caching_dir)

    def build_logging(self, **kwargs):
        self.build_logging_dir()
        for handler in logging.root.handlers[:]:
            logging.root.removeHandler(handler)
        logging.basicConfig(filename=self.log_path+'/running.log', level=logging.INFO,
