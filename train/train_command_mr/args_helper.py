...
def arg_wrapper():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="run attacks each epoch to measure robustness, but train normally",
    )
    parser.add_argument(
        "--num-clean-epochs",
        type=int,
        default=0,
        help="Number of epochs to train on the clean dataset before adversarial training (N/A if --attacks unspecified)",
    )
    parser.add_argument(
        "--attacks-period",
        type=int,
        default=1,
        help="How often (in epochs) to generate a new adversarial training set (N/A if --attacks unspecified)",
    )
    parser.add_argument(
        "--augment",
        type=str,
...
def run(args):
    ...
    from train.train_model.run_training import train_model, train_vib
...
def _adv_path(victim_model_with_dataset, recipe_with_split):
    '''
    拼接对抗样本的完整路径
    :param victim_model_with_dataset:
    :param recipe_with_split:
    :return:
    '''
