...
from datasets_wrapper.dataset import ListDataset
from datasets_wrapper.instance import InputInstance
...
class ClassificationReader(BaseReader):
    ...
    def read_from_file(self, args):
        from train_model.train_args_helpers import dataset_from_local
        train_text, train_labels, eval_text, eval_labels = dataset_from_local(args)
        train_size, eval_size = len(train_text), len(eval_text)
        instances_train, instances_eval = [], []
        for i in range(train_size):
            idx = f'train-{i}'
            if args.dataset == 'mr':
                instances_train.append(InputInstance(idx, train_text[i], None, train_labels[i]))
            elif args.dataset == 'snli':
                premise, hypothesis = train_text[i]
                instances_train.append(InputInstance(idx, premise, hypothesis, train_labels[i]))

        for i in range(eval_size):
            idx = f'eval-{i}'
            if args.dataset == 'mr':
                instances_eval.append(InputInstance(idx, eval_text[i], None, eval_labels[i]))
            elif args.dataset == 'snli':
                premise, hypothesis = eval_text[i]
                instances_eval.append(InputInstance(idx, premise, hypothesis, eval_labels[i]))
    ...
    def get_word_set(self, instances, counter_fitted_file, attack_surface):
        ...
        for instance in instances:
            ...
            try:
                ...
                # For now, ignore things not in attacks surface
